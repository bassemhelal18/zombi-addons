...
def showHosters():
    ...
    if aResult[0] is True:
        ...
        VSlog(cook)
        oRequest.addHeaderEntry('User-Agent', 'Mozilla/5.0 (iPad; CPU OS 13_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/110.0.0.0 Mobile/15E148 Safari/604.1')
        ...
        oRequest.addHeaderEntry('Referer', 'https://m1.arabseed.world/')
        
        
        sHtmlContent = oRequest.request()
        VSlog(sHtmlContent)
    
    sPattern = '<li data-link="([^<]+)" class='
    oParser = cParser()
    aResult = oParser.parse(sHtmlContent, sPattern)
